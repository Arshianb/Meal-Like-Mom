...
# About The Project
When the user becomes a member of our application for the first time, his phone Id will be saved on the database, and after the program deleted by the user, he can log in to his account only by username because his phone id remains in database. The user's phone internal database will be applied on the application, when his Internet connection was lost! for better performance of the user. Database tables are interconnected by Id for accurate classification of cuisine tutorials. The aforementioned is programmed by `Android Studio`, `PHP`.
