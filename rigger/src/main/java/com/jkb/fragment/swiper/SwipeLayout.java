...
/**
 * The widget support {@link AppCompatActivity}/{@link android.support.v4.app.Fragment} to exit by swipe edge.
 * <p>
...
public class SwipeLayout extends FrameLayout {
    ...
    private void computeScrollPreView() {
        Fragment preFragment = getPreFragment();
        if (preFragment == null) {
            if (mPuppetHost instanceof AppCompatActivity) {
    ...
    private void computeScrollActivityView() {
        AppCompatActivity activity = getPreActivity();
        if (activity == null) {
            return;
        }
        View decorView = activity.getWindow().getDecorView();
    ...
    @Nullable
    private AppCompatActivity getPreActivity() {
        if (mPuppetHost instanceof AppCompatActivity) {
            Stack<AppCompatActivity> stack = SwipeActivityManager.getInstance().getActivityStack();
